...
def create_balanced_round_robin_evens(n):
    """
...
def drr_fixture(fixture, teams, inmatchseparator=" v "):
    ...
    return fixture made up of H v A format with tab separator between matches of same round in same line.
...
def save_fixture(n, fixture, fixturenamesuffix=""):
    ...
    #..."FixtureTemplate" can be manually altered to suit
    with open("FixtureTemplate" + str(n) + fixturenamesuffix + ".txt", "w") as f:
        f.write('\n'.join(fixture))
...
def npmatrix_from_fixture(fixture):
...
def fixture_anlaysis(fixture, n):
    npfix = npmatrix_from_fixture(fixture)
    print(npfix)
    #make sure n is even
    if n % 2 == 1:
        n = n + 1
    #init numpy array for length test to start
    arr_col_occs = np.array([])
    for i in range(n):
        column_occurrences = np.count_nonzero(npfix == i, axis=0)
        #print(column_occurrences)
        #print(type(column_occurrences))
        # add each row to numpy.ndarray
        if len(arr_col_occs) == 0:
            arr_col_occs = column_occurrences
        else:
            arr_col_occs = np.vstack((arr_col_occs, column_occurrences))
    return arr_col_occs
...
for n in range(6, 7, 2):
    fixture = create_balanced_round_robin_evens(n)
    drr_fixture = create_drr(fixture)
    print(drr_fixture)
    print(fixture_anlaysis(fixture, n))
    print(fixture_anlaysis(drr_fixture, n))
    teams = teams_number_list(n)
    drr = drr_fixture_withshift(fixture, teams, inmatchseparator=" v ")
    #print(drr)
    # save_fixture(n, drr, fixturenamesuffix='')
